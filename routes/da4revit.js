...
const { ItemsApi, VersionsApi, BucketsApi, ObjectsApi } = require("forge-apis");
...
///////////////////////////////////////////////////////////////////////
router.get(
  "/da4revit/v1/revit/:version_storage/excel",
  async (req, res, next) => {
    ...
    try {
      ...
      const resources = await objectApi.uploadResources(
        bucketKey,
        objects,
        {},
        oauth_client,
...
router.get("/da4revit/v1/revit/:workitem_id", async (req, res, next) => {
  ...
  try {
    ...
    const oauth_client = oauth.get2LeggedClient();
    const oauth_token = await oauth_client.authenticate();
    let workitemRes = await getWorkitemStatus(
      workitemId,
...
router.post("/callback/designautomation", async (req, res, next) => {
  ...
  if (req.body.status === "success") {
    ...
    if (workitem.createVersionData != null) {
      ...
      global.MyApp.SocketIo.emit(SOCKET_TOPIC_WORKITEM, workitemStatus);
      console.log(
        "Starting to post handle the DA workitem:  " + workitem.workitemId
      );
      try {
      ...
      try {
        const objectApi = new ObjectsApi();
        const downloadInfo = await objectApi.getS3DownloadURL(
          workitem.objectInfo.BucketKey,
          workitem.objectInfo.ObjectKey,
          null,
          req.oauth_client,
          workitem.access_token_2Legged
        );
        workitemStatus.Status = "Completed";
