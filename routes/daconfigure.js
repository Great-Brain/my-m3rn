...
router.get("/designautomation/activities", async (req, res, next) => {
  try {
    const activitiesRes = await apiClientCallAsync(
      "GET",
      designAutomation.URL.ACTIVITIES_URL,
      req.oauth_token.access_token
    );
...
router.post("/designautomation/appbundles", async (req, res, next) => {
  ...
  let appBundles = null;
  try {
    const appBundlesRes = await apiClientCallAsync(
      "GET",
      designAutomation.URL.APPBUNDLES_URL,
      req.oauth_token.access_token
    );
  ...
  if (appBundles.includes(qualifiedAppBundleId)) {
    try {
      ...
      const createAppVersionUrl =
        designAutomation.URL.CREATE_APPBUNDLE_VERSION_URL.format(appBundleName);
      newAppVersion = await apiClientCallAsync(
        "POST",
        createAppVersionUrl,
        req.oauth_token.access_token,
        appBundleSpec
      ...
      const modifyAppAliasUrl =
        designAutomation.URL.UPDATE_APPBUNDLE_ALIAS_URL.format(
          appBundleName,
          designAutomation.appbundle_activity_alias
        );
      await apiClientCallAsync(
        "PATCH",
        modifyAppAliasUrl,
        req.oauth_token.access_token,
        aliasSpec
    ...
    try {
      ...
      const createAppBundleAliasUrl =
        designAutomation.URL.CREATE_APPBUNDLE_ALIAS_URL.format(appBundleName);
      await apiClientCallAsync(
        "POST",
        createAppBundleAliasUrl,
        req.oauth_token.access_token,
        aliasSpec
      );
    } catch (err) {
...
router.post("/designautomation/activities", async (req, res, next) => {
  ...
  let activities = null;
  try {
    const activityRes = await apiClientCallAsync(
      "GET",
      designAutomation.URL.ACTIVITIES_URL,
      req.oauth_token.access_token
    );
  ...
  if (!activities.includes(qualifiedActivityId)) {
    const activitySpec = {
      ...
      CommandLine: [
        '$(engine.path)\\\\revitcoreconsole.exe /i "$(args[inputFile].path)" /al "$(appbundles[' +
          appBundleName +
          '].path)"',
      ],
      Engine: engineName,
    ...
    try {
      newActivity = await apiClientCallAsync(
        "POST",
        designAutomation.URL.ACTIVITIES_URL,
        req.oauth_token.access_token,
        activitySpec
      ...
      const createActivityAliasUrl =
        designAutomation.URL.CREATE_ACTIVITY_ALIAS.format(activityName);
      await apiClientCallAsync(
        "POST",
        createActivityAliasUrl,
        req.oauth_token.access_token,
        aliasSpec
      );
    } catch (err) {
...
///////////////////////////////////////////////////////////////////////
router.delete(
  "/designautomation/appbundles/:appbundle_name",
  async (req, res, next) => {
    const appbundle_name = req.params.appbundle_name;
    try {
      await apiClientCallAsync(
        "DELETE",
        designAutomation.URL.APPBUNDLE_URL.format(appbundle_name),
        req.oauth_token.access_token
      );
...
///////////////////////////////////////////////////////////////////////
router.delete(
  "/designautomation/activities/:activity_name",
  async (req, res, next) => {
    const activity_name = req.params.activity_name;
    try {
      await apiClientCallAsync(
        "DELETE",
        designAutomation.URL.ACTIVITY_URL.format(activity_name),
        req.oauth_token.access_token
      );
