...
async function startWorkitem() {
  ...
  if (sourceNode == null || sourceNode.type !== "versions") {
    alert(
      "Can not get the selected file, please make sure you select a version as input"
    );
    return;
  }
  ...
  try {
    ...
    if (exporting) {
      ...
      res = await importExcel(
        sourceNode.original.storage,
        storageUrl,
        inputJson,
        sourceNode.parent,
        fileName
...
function updateStatus(status, extraInfo = "") {
  ...
  switch (status) {
    ...
    case "completed":
      ...
      statusText.innerHTML = exporting
        ? "<h4>Step 3/3: Done, Ready to <a href='" +
          extraInfo +
          "'>DOWNLOAD</a></h4>"
