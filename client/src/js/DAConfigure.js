$(document).ready(function () {
  ...
  $("#clearAccount").click(async () => {
    ...
    try {
      ...
      updateConfigStatus(
        "deleting_completed",
        "{0} & {1}".format(appBundleName, activityName)
      );
    } catch (err) {
      console.log(err);
      updateConfigStatus(
...
async function createAppBundleActivity() {
  ...
  try {
    ...
    updateConfigStatus(
      "creating_completed",
      "{0}AppBundle & {1}Activity".format(fileName, fileName)
    );
  } catch (err) {
    updateConfigStatus(
      "creating_failed",
  ...
  switch (status) {
    case "creating_appbundle":
      ...
      statusText.innerHTML =
        "<h4>Step 1/2: Creating AppBundle: " + info + "</h4>";
    ...
    case "creating_activity":
      ...
      statusText.innerHTML =
        "<h4>Step 2/2: Creating Activity: " + info + "</h4>";
      upgradeBtnElm.disabled = true;
      cancelBtnElm.disabled = true;
      break;
    ...
    case "deleting_appbundle":
      setProgress(20, "configProgressBar");
      statusText.innerHTML =
        "<h4>Step 1/2: Deleting AppBundle: " + info + "</h4>";
      upgradeBtnElm.disabled = true;
      cancelBtnElm.disabled = true;
      break;
    case "deleting_activity":
      setProgress(60, "configProgressBar");
      statusText.innerHTML =
        "<h4>Step 2/2: Deleting Activity: " + info + "</h4>";
      upgradeBtnElm.disabled = true;
      cancelBtnElm.disabled = true;
      break;
